...
class MainWindow:
    def __init__(self):
        ...
        self.main.title("Generator Haseł")
        self.widgets()
        self.main.mainloop()
    def widgets(self):
        self.frame = tk.Frame(self.main)
        self.val,self.special,self.num=tk.IntVar(), tk.IntVar(), tk.IntVar()
        self.optionsFrame = tk.LabelFrame(self.frame, text="Ustawienia")
        self.uppercase = tk.Checkbutton(self.optionsFrame, text= "Duże litery", onvalue=1, offvalue=0, variable=self.val)
        self.specialChar =tk.Checkbutton(self.optionsFrame, text= "Znaki specjalne", onvalue=1, offvalue=0, variable=self.special)
        self.number = tk.Checkbutton(self.optionsFrame, text= "Numer", onvalue=1, offvalue=0, variable=self.num)


        self.length = tk.IntVar()
        self.lengthFrame = tk.LabelFrame(self.frame, text="Długość Hasła")
        ...
        self.genPwd = tk.Button(self.main, text="Wygeneruj Hasło", width=25, command=self.generatePassword)
        self.viewHistory = tk.Button(self.main, text="Zobacz Historię", width=25, command=self.getHistory)
